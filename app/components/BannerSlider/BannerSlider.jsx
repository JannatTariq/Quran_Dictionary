      ...
      >
        <SwiperSlide>
            <div className="relative h-full w-full isolate">
                <Image className="w-full h-full object-contain lg:object-cover absolute bg-gradient-to-r from-[#cb9918] to-[#e5bd52]" src={BG1}/>
                ...
                <div className="text-white relative z-10 px-8 py-8 lg:py-12 h-full flex flex-col">
                    <h2 className="text-base lg:text-6xl text-black text-center mt-5 lg:mt-[110px] lg:mb-10 font-hafs font-bold" style={{textShadow:"0px 0px 2px #fff"}}>إِنَّا أَنزَلْنَاهُ قُرْآنًا عَرَبِيًّا لَّعَلَّكُمْ تَعْقِلُونَ</h2>
                </div>
            </div>
        </SwiperSlide>
        {categories.length ? categories.map((category, i)=>{
          const matches = category.name?.trim().match(/^(.+?)\s*\(([^()]+)\)\s*$/);
          const title = matches?.[1] ?? category.name ?? "";
          const subtitle = matches?.[2] ?? "";
          return (
        <SwiperSlide key={category.id}>
            <div className="relative h-full w-full isolate">
                <Image className="w-full h-full object-cover absolute" src={bannerImages[i]}/>
                <div className="absolute inset-0 bg-primary-900 opacity-50"></div>
                <div className="text-white text-center relative z-10 px-8 py-12 h-full flex flex-col justify-center items-center gap-2 lg:gap-8">
                    ...
                    <Link href={`/subcategory?categoryId=${category._id}&categoryName=${category.name}&index=${i}`} className="block px-4 py-1 rounded-full bg-secondary text-white text-[10px] lg:text-xl hover:bg-secondary-400">Start reading</Link>
                </div>
            </div>
        </SwiperSlide>
        )
