...
function Jumbotron({ title = "Page Title", categoryId, total, isSearch }) {
  const pathname = usePathname();
  const paths = pathname.split("/").filter((path) => path !== "");
  if (isSearch) {
    return (
      <div
        className="relative py-4 px-4 lg:py-[100px] lg:px-12 bg-bottom bg-fixed"
        style={{ backgroundImage: `url(/images/search-banner.jpg)` }}
      >
        ...
        <div className="relative z-10">
          <div className="flex gap-2 text-sm lg:text-base bg-overlay w-fit leading-4 py-1 px-2">
            <Link
              href={`/`}
              className="capitalize text-secondary-400 hover:text-secondary-300 font-bold"
            >
              {"Home >>"}
            </Link>
            {paths.map((path, i) => (
              <Link
                key={path}
                href={i !== paths.length - 1 ? `/${path}` : ""}
                className={`capitalize font-bold ${i !== paths.length - 1 ? "text-secondary-400 hover:text-secondary-300" : "text-white"}`}
              >
                {path}
              </Link>
            ))}
    ...
    return (
      <div
        className="relative py-4 px-4 lg:py-[100px] lg:px-12 bg-bottom bg-fixed bg-cover bg-no-repeat"
        style={{ backgroundImage: `url(/images/islamic-banner.jpg)` }}
      >
        <div className="absolute inset-0 bg-primary-800 opacity-50"></div>
        <div className="relative z-10">
          <div className="flex gap-2 text-sm lg:text-base bg-overlay w-fit leading-4 py-1 px-2">
          ...
          <div className="text-center text-white">
            ...
            <h3 className="text-sm lg:text-xl mb-4">
              {total ? `${total.toLocaleString()} items` : "Loading..."}
            </h3>
